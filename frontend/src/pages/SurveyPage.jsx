  ...
  return (
    <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
      <span style={{
        ...
        borderRadius: 6, padding: "4px 0", flexShrink: 0,
      }}>P{index}</span>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          ...
          {query && (
            <button onClick={clear} style={{
              ...
              color: "var(--color-text-tertiary)", fontSize: 16, padding: "0 4px", lineHeight: 1,
            }}>×</button>
          )}
        </div>
        {open && (
          <div style={{
            ...
            {results.map(s => (
              <div key={s.enrollment} onMouseDown={() => select(s)}
                style={{ padding: "9px 14px", cursor: "pointer", fontSize: 13, borderBottom: "1px solid var(--color-border-tertiary)" }}
...
export default function SurveyPage() {
  ...
  useEffect(() => {
    ...
    api.getStudents().then(d => setAllStudents(d.students)).catch(() => {}).finally(() => setLoadingStudents(false));
  ...
  const handleSubmit = async () => {
    ...
    prefs.forEach((p, idx) => {
      if (p.enrollment && p.enrollment !== enrollment)
        preferences[p.enrollment] = idx + 1;
    });
    ...
    try {
      ...
      setSubmitted(true);
      setSurveyStatus(await api.getSurveyStatus());
  ...
  if (submitted) return (
    <div style={{ padding: 40, maxWidth: 560, margin: "0 auto", textAlign: "center" }}>
      <div style={{
        ...
        margin: "0 auto 20px", fontSize: 24, color: "#085041",
      }}>✓</div>
      ...
      {surveyStatus && (
        <div style={{
          marginTop: 24, padding: "12px 20px", background: "var(--color-background-secondary)",
  ...
  return (
    <div style={{ padding: "28px 24px", maxWidth: 680, margin: "0 auto" }}>
      ...
      <p style={{ color: "var(--color-text-secondary)", marginBottom: 24, fontSize: 14 }}>
        List your friends in order of preference. Row 1 = highest priority.
      </p>

      {surveyStatus && (
        <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
          {[{ l: "Submitted", v: surveyStatus.submitted }, { l: "Pending", v: surveyStatus.pending }, { l: "Total", v: surveyStatus.total_students }].map(({ l, v }) => (
            <div key={l} style={{ flex: 1, background: "var(--color-background-secondary)", border: "1px solid var(--color-border-tertiary)", borderRadius: 10, padding: "12px 16px" }}>
      ...
      <div style={{
        background: "var(--color-background-secondary)", border: "1px dashed var(--color-border-secondary)",
