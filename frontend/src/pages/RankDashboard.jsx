...
const COHORT_PRESETS = [200, 400, 576, 600, 800, 1000];
...
export default function RankDashboard() {
  ...
  const [cohortSize, setCohortSize] = useState(576);
  ...
  const handleDummy = async (size) => {
    const n = size || cohortSize;
    setDummy(true); setError(""); setSuccess("");
    try {
      const r = await devApi.loadDummy(n);
      const d = await api.getStudents();
      setStudents(d.students); setPage(0);
  ...
  const applyCustomSize = () => {
    ...
    if (n >= 10 && n <= 2000) {
      ...
      setError("Cohort size must be between 10 and 2,000");
    }
  };
  ...
  const filtered = useMemo(() => {
    ...
    const list = q
      ? students.filter(s => s.name.toLowerCase().includes(q) || s.enrollment.includes(q) || (s.section || "").toLowerCase().includes(q))
      : students;
  ...
  return (
    <div style={{ padding: "28px 24px", maxWidth: 1100, margin: "0 auto" }}>
      ...
      <p style={{ color: "var(--color-text-secondary)", fontSize: 14, marginBottom: 24 }}>
        Upload a JSON file (name, enrollment_number, sgpa) to load student data, or use dummy data.
      </p>
      ...
      <Card style={{ marginBottom: 20 }}>
        <div style={{ display: "flex", gap: 20, flexWrap: "wrap", alignItems: "flex-end" }}>
          ...
          <div style={{ borderLeft: "1px solid var(--color-border-tertiary)", paddingLeft: 20, flex: "1 1 auto" }}>
            <div style={{ fontSize: 11, color: "var(--color-text-tertiary)", marginBottom: 6 }}>Dev mode — Cohort size</div>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center", marginBottom: 8 }}>
              {COHORT_PRESETS.map(size => (
                <button
                  key={size}
                  onClick={() => selectPreset(size)}
                  style={{
                    padding: "5px 12px", borderRadius: 20, border: "1px solid",
                    borderColor: !showCustom && cohortSize === size ? "var(--color-accent)" : "var(--color-border-secondary)",
                    background: !showCustom && cohortSize === size ? "var(--color-accent)" : "transparent",
                    color: !showCustom && cohortSize === size ? "#fff" : "var(--color-text-secondary)",
              ...
              <button
                onClick={() => { setShowCustom(true); setCohortSize(0); }}
                style={{
                  padding: "5px 12px", borderRadius: 20, border: "1px solid",
                  borderColor: showCustom ? "var(--color-accent)" : "var(--color-border-secondary)",
                  background: showCustom ? "var(--color-accent)" : "transparent",
            ...
            {showCustom && (
              <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 8 }}>
                <input
                  type="number" min={10} max={2000} placeholder="10 – 2000"
                  value={customSize} onChange={e => setCustomSize(e.target.value)}
                  onKeyDown={e => e.key === "Enter" && applyCustomSize()}
                  style={{
                    padding: "5px 10px", borderRadius: 8, border: "1px solid var(--color-border-secondary)",
                    fontSize: 13, width: 100, background: "var(--color-background-primary)",
                    color: "var(--color-text-primary)",
                  }}
                />
                <Btn variant="secondary" size="sm" onClick={applyCustomSize}>Set</Btn>
              </div>
            )}
            <Btn variant="ghost" onClick={() => handleDummy()} disabled={dummyLoading || (!showCustom && !cohortSize) || (showCustom && !cohortSize)}>
              {dummyLoading ? "Generating…" : `Generate ${cohortSize || "?"} students`}
            </Btn>
          </div>
        </div>
      </Card>
      ...
      {loading ? (
        ...
        <Card style={{ textAlign: "center", padding: "48px 24px" }}>
          <p style={{ fontSize: 13, color: "var(--color-text-tertiary)" }}>No data loaded. Upload files or generate dummy students above.</p>
        </Card>
      )}
    </div>
  );
