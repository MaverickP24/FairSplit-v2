    ...
    <div style={{padding:"28px 24px",maxWidth:900,margin:"0 auto"}}>
      ...
      {loading && !metrics && (
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {[1,2,3,4].map(i=><Card key={i}><Skeleton height={16} width="50%" style={{marginBottom:8}}/><Skeleton height={10}/><Skeleton height={6} style={{marginTop:4,opacity:.5}}/></Card>)}
        </div>
      )}
      ...
      {metrics && !loading && (
        <>
          ...
          <Card style={{marginBottom:16}}>
            ...
            <CompareBar label="Satisfaction score" value={metrics.satisfaction_score}
              baseline={metrics.baseline_satisfaction_score} higherIsBetter/>
            <CompareBar label="Isolation rate" value={metrics.isolation_rate}
              baseline={metrics.baseline_isolation_rate} higherIsBetter={false}/>
          </Card>
          ...
          <Card style={{marginBottom:16}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              ...
              <div style={{fontSize:24,fontWeight:500}}>
                ...
                <span style={{fontSize:13,fontWeight:400,color:"var(--color-text-secondary)"}}> pts</span>
              </div>
            </div>
          ...
          <Card>
            ...
            <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
              {Object.entries(metrics.section_sizes||{}).map(([name,size])=>{
                const pal = SECTION_PALETTE[name]||SECTION_PALETTE.A;
                return (
                  <div key={name} style={{flex:"1 1 100px",background:pal.bg,
                    border:`1px solid ${pal.border}`,borderRadius:10,padding:"14px 18px",textAlign:"center"}}>
